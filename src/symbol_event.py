...
class SymbolEvent:
    ...
    def __init__(self, symbol: str, event_dates: list[date], pre_event: int, post_event: int, max_offset: int) -> None:
        ...
        self.start = min(event_dates) - timedelta(days=(pre_event + max_offset))
        self.end = max(event_dates) + timedelta(days=post_event)

        assert self.end <= date.today()

        self.csv_filepath = data_fetch.fetch_to_csv(symbol, self.start, self.end)
        self.raw_df, self.event_dates_indices = data_analysis.raw_df(self.csv_filepath, event_dates)
        self.raw_tensor = data_analysis.raw_tensor(self.raw_df)
